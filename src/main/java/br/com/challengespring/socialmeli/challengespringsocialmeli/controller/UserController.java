...
public class UserController {
    ...
    public ResponseEntity<FollowedSellerDTO> followUser(@RequestHeader @Validated Long idUser, @RequestHeader @Validated Long userIdToFollow){
        FollowedSellerDTO user = service.follow(idUser, userIdToFollow);
        if (user!=null){
            return new ResponseEntity<FollowedSellerDTO>(user, HttpStatus.OK);
        } else {
            return new ResponseEntity<FollowedSellerDTO>(HttpStatus.BAD_REQUEST);
        }
    }
    ...
    public ResponseEntity<FollowedSellerDTO> unfollowUser(@RequestHeader @Validated Long idUser, @RequestHeader @Validated Long userIdToUnfollow){
        Optional<FollowedSellerDTO> userIn = Optional.ofNullable(service.unfollow(idUser, userIdToUnfollow));
        if (!userIn.isPresent()){
            return new ResponseEntity<FollowedSellerDTO>(HttpStatus.BAD_REQUEST);
        } else {
            return new ResponseEntity<FollowedSellerDTO>(userIn.get(), HttpStatus.OK);
        }
    }
